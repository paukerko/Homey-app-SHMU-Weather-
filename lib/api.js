...
function extractDatasetsFromHtml(html) {
  ...
  try {
    ...
    try {
      const datasets = new Function('"use strict"; return (' + datasetsJson + ')')();
      if (Array.isArray(datasets) && datasets.length > 0) {
        console.log(
      ...
      console.log(
        "[api] Failed with Function eval too:",
        evalError.message,
      );
      console.log("[api] First 400 chars of extracted data:", datasetsJson.substring(0, 400));
    }
  }
