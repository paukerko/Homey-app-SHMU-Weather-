...
function parseHourlyDatasets(datasets) {
  ...
  try {
    ...
    const tempDataset = datasetById.get("ttt") || datasets[0];
    const pressureDataset = datasetById.get("tlak") || datasets[1];
    const humidityDataset = datasetById.get("rh") || datasets[2];
    const windSpeedDataset = datasetById.get("ff") || datasets[3];
    const windGustDataset = datasetById.get("fm") || null;
    const rainfallDataset =
      datasetById.get("pr1h") || datasets[datasets.length - 1];
    ...
    const windGust = windGustDataset
      ? findLatestValidMeasurement(windGustDataset.data)
      : null;
